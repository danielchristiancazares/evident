...
It also includes package-scale HIR comparison, MIR golden-output comparisons, LLVM IR golden-output comparisons, CLI help/version/toolchain golden-output comparisons, CI workflow contract validation for the release artifact path, assembly/object emission smoke tests, COFF object header and section-table validation, PE executable header and section-table validation, install-layout and extracted package-ZIP validation including README/license/docs contents, linked executable exit-code tests, and backend entrypoint rejection tests.
