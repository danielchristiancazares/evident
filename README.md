...
The first bootstrap-oriented Evident package outline lives under `bootstrap/compiler`. It is a compilable scaffold for stage-oriented self-hosting work, with package identity, a deterministic source graph, and initial lexical token stream and parsed package syntax models, not a self-hosted compiler implementation.
