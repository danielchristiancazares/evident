...
The first bootstrap-oriented Evident package outline lives under `bootstrap/compiler`. It is a compilable scaffold for stage-oriented self-hosting work, with package identity, a deterministic source graph, and an initial lexical token stream model, not a self-hosted compiler implementation.
