...
The first bootstrap-oriented Evident package outline lives under `bootstrap/compiler`. It is a compilable scaffold for stage-oriented self-hosting work, with package identity, a deterministic source-discovery plan, an original-file diagnostic anchor, a bootstrap stage harness with structured stage tool invocations and output paths, a release evidence record, a deterministic package manifest and layout with an internal dependency graph, and initial lexical token stream, parsed package syntax, and semantic symbol catalog models, not a self-hosted compiler implementation. Its package identity now comes from the bootstrap environment surface, and it carries host file, path, environment, and tool boundary models plus bootstrap stage-provenance, equivalence, and structured stage tool invocation/output-path records so the scaffold can grow toward a documented stage-1 / stage-2 proof. A native probe now also links the bootstrap environment foreign hook, exercises bootstrap package-validation and release-evidence construction, and a host-runtime smoke probe exercises path resolution, text file read/write, and process invocation in native output.
