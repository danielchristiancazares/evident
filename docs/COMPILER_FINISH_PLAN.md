...
- Major bootstrap gaps remain: `bootstrap/compiler` is only an initial Evident package outline with package identity, a deterministic source-discovery plan, an original-file diagnostic anchor, lexical token stream model, parsed package syntax model, and semantic symbol catalog model, not a compiler implementation. It now also models the host runtime, environment, and bootstrap stage-provenance/equivalence records, and the package identity is derived from the bootstrap environment surface. The `run_bootstrap_compiler_scaffold` CTest keeps that scaffold natively compilable with the seed compiler, but there is no bootstrap stage harness, no release evidence section for seed, stage-1, and stage-2 provenance, and the language/runtime boundary for compiler-scale file I/O, diagnostics, path handling, and tool invocation is not yet complete.
