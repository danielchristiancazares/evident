...
- The language specification now defines the completed compiler-owned collection operation surface. The current compiler recognizes the collection families, injects companion reasons, companion records, and compiler-owned operation signatures through semantic analysis and HIR, enforces map-key restrictions and copyability availability rules, emits native zero-length ABI carriers for `list_empty` and `map_empty`, widens non-empty collection carriers, lowers collection count operations from the ABI length field, returns cardinality failures for `list_require_nonempty` and `map_require_nonempty`, stores single-element lists for `list_single`, stores single-entry maps for `map_single`, scans, updates, removes, consumes, merges, and builds map entries for `map_lookup_copy`, `nonempty_map_lookup_copy`, `map_bind_new`, `nonempty_map_bind_new`, `map_replace_bound`, `nonempty_map_replace_bound`, `map_bind_or_replace`, `nonempty_map_bind_or_replace`, `map_remove_bound`, `nonempty_map_remove_bound`, `map_consume_bound_value`, `nonempty_map_consume_bound_value`, the map merge family, and the map-from-entries family, loads first elements for `nonempty_list_first_copy`, loads first map entries for `nonempty_map_first_entry_copy`, builds new lists with `list_prepend`, `list_append`, and the list concat family, consumes the first list element into `ListFirstAndRest<T>` for `nonempty_list_consume_first`, consumes the first map entry into `MapFirstEntryAndRest<K, V>` for `nonempty_map_consume_first_entry`, converts map carriers to list-of-entry carriers for `map_entries_copy`, `nonempty_map_entries_copy`, `map_consume_entries`, and `nonempty_map_consume_entries`, and regression-tests arity, ABI-carrier coverage, HIR recognition, static rejections, supported empty/widen/count/require-nonempty/single/first-copy/lookup/map-bind-update/map-remove/map-consume-bound/map-merge/map-from-entries/prepend/append/concat/consume-first/map-entry-list collection emission, and compiler-owned collection-operation contract diagnostics.
- The language specification now defines structural `traverse` over `List<T>` and `NonEmptyList<T>` as a core accumulator-threading expression. The current compiler does not yet parse, type-check, lower, native-emit, or regression-test `traverse`.
- The language specification now defines UTF-8 source text and string-literal syntax in Section 8.1. The compiler rejects ill-formed UTF-8 source files before lexing, rejects source input containing U+0000, rejects unescaped line terminators and unescaped U+0000 in string literals, accepts only the specified string escapes, rejects malformed `\u{H}` escapes, rejects Unicode escapes above U+10FFFF or in the surrogate range, rejects string literals containing U+0000 when `CString` is expected, and decodes valid Unicode escapes into UTF-8 for native string materialization. The current compiler still needs full source-text Unicode scalar decoding, deriving scalar counts for literal typing, and broader non-ASCII string literal regression tests.
- The language specification now also defines the compiler-owned `Text` and `Bytes` scalar-sequence operation surface in Section 9.8: length (`text_length`, `bytes_length`), bounds-checked element access (`text_character_at`, `bytes_byte_at`), and half-open slicing (`text_slice`, `bytes_slice`), together with the compiler-owned `TextBoundsFailure` and `BytesBoundsFailure` bounds reasons. The current compiler recognizes, type-checks, lowers, native-emits, and regression-tests the total length subset (`text_length`, `bytes_length`). Bounds-checked element access, half-open slicing, companion bounds-reason injection, and regression tests for the remaining `Text`/`Bytes` operation surface remain open work.
- The specification additionally defines the copyable `NonEmptyText` and `NonEmptyBytes` built-in types (Section 9.8) with non-empty string-literal typing, static rejection of empty literals in `NonEmptyText`/`NonEmptyBytes` positions, the compiler-owned `TextCardinalityFailure`/`BytesCardinalityFailure` reasons, and the `text_require_nonempty`, `nonempty_text_widen`, `nonempty_text_length`, `nonempty_text_first_character` operations plus their `bytes`/`byte` analogues. These mirror the `List<T>`/`NonEmptyList<T>` cardinality pattern (require-nonempty plus widen plus total first-element access). The compiler now recognizes `NonEmptyText` and `NonEmptyBytes`, types non-empty string literals as those types in expected-type positions, rejects empty literals in those positions, rejects them as map keys, and lowers/native-emits literal values with the same carrier shape as `Text` and `Bytes`. Companion cardinality-reason injection and the non-empty text/bytes operation surface remain open work.
- The specification now defines the strict foreign ABI substrate surface for `CString`, `CInt`, and `CSize`, the compiler-owned `ForeignAbiTextFailure` / `ForeignAbiIntegerFailure` reasons, explicit `CString` and integer conversion operations, and structural raw-substrate / semantic-claim anti-laundering lint rules. The current compiler rejects package-wide duplicate `foreign fn` link names. It does not yet enforce raw-adapter exposure classification, exported domain/foundation raw exposure rejection, foreign-ABI-admissible `foreign fn` signatures, exact foreign symbol binding, target-specific ABI representation and C-prototype matching, out-of-band foreign status/result-channel exclusions, adapter-local reason restrictions, source-visible `CString` rules, exact boundary collapse checks, direct-body-only foreign egress preparation, egress source-carrier and consequence-to-ABI mapping checks, bare primitive/sequence escape rejection, public raw-map and raw-sequence API rejection, closed identifier word normalization and marker-family classification, consequence-carrying type classification, definite consequence containment, exported function-level semantic-claim success-surface checks, primitive/sequence-backed nominal anti-laundering checks, permit type-only authority coverage, foundation semantic-blindness at all visibilities, or the required diagnostics and regression tests. External `CString` lifetime, mutability, stability, storage, cardinality, and release assertions still need audit documentation.
- The specification admits only the `x86_64-pc-windows-msvc` native target in this draft, publishes exact value sets for `Int`, `CInt`, and `CSize` on that target, publishes the target-specific `CInt` / `CSize` / `CString` / `Unit` foreign ABI representation table, defines unbounded `Nat`, defines target-independent `Byte` as 0 through 255 inclusive, defines `Char` exactly as Unicode scalar values, and the new foreign integer and bytes/C-string operations depend on those ranges and representations for exact rejection behavior. The compiler now types integer literals as `Nat`, `Byte`, or, inside `boundary` and `hazard` modules only, `CInt` / `CSize` in expected-type positions, and rejects `Byte`, `CInt`, and `CSize` literals outside their admitted ranges. The current implementation still needs a representation strategy for unbounded `Nat`, exact compiler-owned cardinality results, and the associated conversion-boundary tests. Regression-testing unsupported target rejection, broader numeric range rejection, underflow, overflow, negative-to-`Nat` / `CSize`, exact-boundary conversions, non-scalar `Char` rejection, NUL-byte handling, finite-cardinality admission for `Text`, `Bytes`, collections, and `CString` payloads, exact foreign ABI C-prototype matching, and exact unbounded `Nat` lengths and counts remain open work.
- The specification now admits `Float` as a finite-only IEEE 754 binary64 value type: NaN and the infinities are not admitted values, signed zero is normalized to a single zero, results round under roundTiesToEven, and any literal, operation, conversion, or ingress that would produce NaN or an infinity MUST be rejected with a `reason` or collapsed at the producing site (Section 9). `Float` is an admitted built-in type name and an admitted map key; user code still MUST NOT redeclare it. The compiler now accepts finite float literals, rejects literals that round to infinity, admits `Float` map keys, and lowers `Float` map-key equality/order for the compiler-owned map operations. The `Float` arithmetic, comparison, transcendental, rounding-mode-selection, conversion operation surface, and foreign ABI float binding remain deferred (Section 18).
- The specification now makes `foundation module` declaration permissions exhaustive: only `record` and `fn` declarations are allowed, and `state`, `reason`, `proof`, `permit`, `phase`, and `foreign fn` are rejected at any visibility. The current compiler enforces this with module-kind diagnostics and registered rejection tests, including the private-by-default foundation declaration-kind fixture.
- The specification now rejects inline by-value representation containment cycles across records, states, proofs, phases, compiler-owned companion records, and instantiated generic records. Compiler-owned collection element, key, and value types are representation boundaries for this inline-layout rule, though they remain subject to the separate category, raw-substrate, map-key, affine, and exported-surface rules. The current compiler enforces this with semantic diagnostics for direct, generic, state, proof, phase, and compiler-owned companion-record cycles, while preserving recursive value shapes through compiler-owned collections.
- The specification now treats hazard runtime looseness alongside foreign ambiguity: status-code sentinels, invalid or zero handles, partial runtime states, callback-order uncertainty, unstable resource lifetimes, and provider-selected fallback policy must collapse before non-raw hazard exports expose consequence-carrying handles, proofs, phases, states, reasons, or scoped permit grants. The current compiler does not yet enforce these hazard-collapse rules; module-aware semantic checks and rejection tests remain open work.
...
- Native emission is currently limited to `x86_64-pc-windows-msvc`, and unsupported target selection is regression-tested as an explicit backend diagnostic and reject smoke test.
