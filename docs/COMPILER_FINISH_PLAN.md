...
- The specification now makes `foundation module` declaration permissions exhaustive: only `record` and `fn` declarations are allowed, and `state`, `reason`, `proof`, `permit`, `phase`, and `foreign fn` are rejected at any visibility. The current compiler still needs module-kind diagnostics and regression tests for private foundation category declarations.
