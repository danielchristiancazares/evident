...
- The language specification now defines the completed compiler-owned collection operation surface. The current compiler recognizes the collection families, injects companion reasons, companion records, and compiler-owned operation signatures through semantic analysis and HIR, enforces map-key restrictions and copyability availability rules, emits native zero-length ABI carriers for `list_empty` and `map_empty`, and regression-tests arity, ABI-carrier coverage, HIR recognition, static rejections, supported empty collection emission, and native backend rejection for unsupported compiler-owned operation bodies. Runtime implementation of non-empty collection operations remains open.
