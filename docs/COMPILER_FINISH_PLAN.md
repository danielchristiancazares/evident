...
- The language specification now also defines the compiler-owned `Text` and `Bytes` scalar-sequence operation surface in Section 9.8: length (`text_length`, `bytes_length`), bounds-checked element access (`text_character_at`, `bytes_byte_at`), and half-open slicing (`text_slice`, `bytes_slice`), together with the compiler-owned `TextBoundsFailure` and `BytesBoundsFailure` bounds reasons. `Text` operations address Unicode scalar values and `Bytes` operations address bytes. The current compiler does not yet recognize, type-check, lower, or native-emit these operations; semantic recognition, companion bounds-reason injection, HIR/MIR lowering, native emission, and regression tests for the `Text`/`Bytes` operation surface remain open work, parallel to the already implemented collection-operation surface.
