...
- The language specification now defines UTF-8 source text and string-literal syntax in Section 8.1. The current compiler does not yet fully enforce those source and lexical rules: rejecting ill-formed UTF-8 input, decoding Unicode scalar values, rejecting unescaped line terminators and U+0000 in string literals, accepting only the specified escapes, rejecting invalid `\u{H}` ranges and surrogate escapes, deriving scalar and UTF-8 byte sequences for literal typing, and regression-testing those cases remain open work.
