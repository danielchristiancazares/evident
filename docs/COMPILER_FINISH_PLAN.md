...
- A local x64 Visual Studio developer-shell run of `ctest --preset windows-x64-ninja --output-on-failure` passed `363/363` tests.
...
- The language specification now defines the completed compiler-owned collection operation surface. The current compiler recognizes the collection families, injects companion reasons, companion records, and compiler-owned operation signatures through semantic analysis and HIR, enforces map-key restrictions and copyability availability rules, and regression-tests arity, ABI-carrier coverage, HIR recognition, static rejections, and native backend rejection for unsupported compiler-owned operation bodies. Runtime implementation of collection operations remains open.
