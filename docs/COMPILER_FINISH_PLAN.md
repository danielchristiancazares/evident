...
- The language specification now defines UTF-8 source text and string-literal syntax in Section 8.1. The compiler rejects ill-formed UTF-8 source files before lexing, rejects unescaped line terminators and unescaped U+0000 in string literals, accepts only the specified string escapes, rejects malformed `\u{H}` escapes, rejects Unicode escapes above U+10FFFF or in the surrogate range, rejects string literals containing U+0000 when `CString` is expected, and decodes valid Unicode escapes into UTF-8 for native string materialization. The current compiler still needs full source-text Unicode scalar decoding, byte-level regression coverage for unescaped U+0000 source input, deriving scalar counts for literal typing, and broader non-ASCII string literal regression tests.
...
- The specification additionally defines the copyable `NonEmptyText` and `NonEmptyBytes` built-in types (Section 9.8) with non-empty string-literal typing, static rejection of empty literals in `NonEmptyText`/`NonEmptyBytes` positions, the compiler-owned `TextCardinalityFailure`/`BytesCardinalityFailure` reasons, and the `text_require_nonempty`, `nonempty_text_widen`, `nonempty_text_length`, `nonempty_text_first_character` operations plus their `bytes`/`byte` analogues. These mirror the `List<T>`/`NonEmptyList<T>` cardinality pattern (require-nonempty plus widen plus total first-element access) and are likewise spec-defined but not yet implemented: builtin-type recognition, non-empty literal typing and the empty-literal rejection, companion cardinality-reason injection, HIR/MIR lowering, native emission, rejection tests for `Map<NonEmptyText, ...>` / `Map<NonEmptyBytes, ...>`, and regression tests remain open work.
