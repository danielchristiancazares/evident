...
The repository now contains an Evident package outline at `bootstrap/compiler`. That package is a compilable bootstrap scaffold, not a compiler implementation. The initial Evident package outline at `bootstrap/compiler` models package identity, a deterministic source-discovery plan, an original-file diagnostic anchor, a bootstrap stage harness with structured stage tool invocations and output paths, a release evidence record, a deterministic package manifest and layout with an internal dependency graph, initial lexical token stream data, parsed package syntax data, and a semantic symbol catalog that together cover the full bootstrap package graph so stage-oriented self-hosting work has real Evident source and compiler-domain data to carry through the pipeline while the larger language and runtime gaps close. Its package validation now records discovery, token, syntax, symbol, source-file, module, and manifest-entry counts as part of the release evidence chain. The bootstrap validation path also generates a build-tree copy of the environment module with the current git source commit embedded so release evidence carries actual provenance instead of a static placeholder. Its package identity is derived from the bootstrap environment surface, and it carries host file, path, environment, and tool boundary data plus bootstrap stage-provenance, equivalence, and structured stage tool invocation/output-path records. The Windows CTest suite emits and runs this scaffold through `run_bootstrap_compiler_scaffold` so seed-compiler validation keeps the package natively compilable. A native probe now also links the bootstrap environment foreign hook, exercises bootstrap package-validation and release-evidence construction, and a host-runtime smoke probe exercises path resolution, text file read/write, and process invocation in native output.
